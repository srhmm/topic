...
from topic.util.embedding import HermiteEmbedding
...
class GaussianProcessFourierRegularized(GaussianProcessRegressor):
    ...
    def fit(self, X, y):
        ...
        if self.fourier_type == FourierType.QUADRATURE:
            ...
            emb = HermiteEmbedding(gamma=0.5, m=self.n_basisf_or_components, d=self.X_train_.shape[1], groups=None,
                                   approx="hermite")  # Squared exponential with lenghtscale 0.5 with m basis functions
            ...
            rbf_feature = RBFSampler(gamma=1, random_state=1, n_components=self.n_basisf_or_components)
